...
## GitHub リポジトリ構成
- リポ: `mametsubu8/AnimatorController_MA_Context`（public）
- Git: `core.worktree = Assets/AnimatorControllerMAContext`（GitHub上ではパッケージ内がルートに見える）
- CLAUDE.md は `.gitignore` で除外（内部開発メモのため非公開）
- 公開用ファイル: CONTRIBUTING.md, CODE_OF_CONDUCT.md, SECURITY.md, .github/ISSUE_TEMPLATE/
- 統合VPMリスティング: `mametsubu8/vpm-listing`（mametsubu全パッケージ共通、6時間ごと自動再ビルド）
